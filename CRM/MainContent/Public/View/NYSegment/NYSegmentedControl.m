...
- (void)moveSelectedSegmentIndicatorToSegmentAtIndex:(NSUInteger)index animated:(BOOL)animated {
    ...
    if (animated) {
        [UIView animateKeyframesWithDuration:self.segmentIndicatorAnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
...
- (void)setSelectedSegmentIndex:(NSUInteger)selectedSegmentIndex animated:(BOOL)animated {
    ...
    if (self.sendBlock) {
        self.sendBlock(_selectedSegmentIndex);
    }
}
