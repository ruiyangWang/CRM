...
//文件上传
+ (void)uploadWithURL:(NSString *)urlSTring cBlock:(Completion)cblock fBlock:(FailBlock)fblock
{
    NSURL *url = [NSURL URLWithString:urlSTring];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    request.HTTPMethod = @"POST";
    ...
    NSURLSession *session = [NSURLSession sharedSession];
    NSURLSessionUploadTask *sessionUploadTask = [session uploadTaskWithRequest:request fromData:[NSData data] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
