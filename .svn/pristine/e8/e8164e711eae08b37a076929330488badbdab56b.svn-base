...
- (void)createView
{
    ...
    title = @[@"",@"联系电话",@"出生日期",@"身份证号",@"微信/QQ",@"",@"保有车型",@"居住地址"];
...
- (void)setupViewDataWithCustomerInfo
{
    ...
    [_subMDTitle setValue:_custListModel.idNumber forKey:@"身份证号"];
    [_subMDTitle setValue:_custListModel.qq forKey:@"微信/QQ"];
    ...
    if ([_custListModel.customerStatus integerValue] == 3) {
        ...
        if ([_custListModel.customerStatus integerValue] == 4) {
            ...
            self.footView.timeLabel.text = [_orderModel.overTimeString substringToIndex:10];
...
{
    YDGiveCarVC *vc = [[YDGiveCarVC alloc] init];
...
{
    YDCancelOrderVC *vc = [[YDCancelOrderVC alloc] init];
    vc.cModel = _custListModel;
    [self.navigationController pushViewController:vc animated:YES];
}
...
{
    NSDictionary *dic = @{@"customerId" : _customerID};
    ...
    [YDDataService startRequest:dic url:url block:^(id result) {
        if ([result[@"code"] isEqualToString:@"S_OK"]) {
            ...
            _custListModel = [YDCustListModel instanceWithDict:var[@"customerInfo"]]; //客户信息
...
//    }
    ...
    [customerInfo setValue:_subMDTitle[@"微信/QQ"] forKey:@"weixin"];
    [customerInfo setValue:_subMDTitle[@"微信/QQ"] forKey:@"qq"];
...
//    _carRel = @{@"brandsId":_custListModel.brandName, @"carsId":@(ctID), @"carTypeId":@(cID), @"type":@0};
    ...
    [YDDataService startRequest:mDic url:url block:^(id result) {
        ...
        if ([[result objectForKey:@"code"] isEqualToString:@"S_OK"]) {
            [MBProgressHUD showTips:@"修改成功" toView:self.view];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == _userInfoTV) {
        ...
        cell.EndEditBlack = ^(YDUpdateCusInfoS2Cell *cell , NSString *str){
            if ([cell.titleString isEqualToString:@"身份证号"]) {
                _subMDTitle[@"身份证号"] = str;
            } else if ([cell.titleString isEqualToString:@"微信/QQ"]) {
                _subMDTitle[@"微信/QQ"] = str;
            }
        };
