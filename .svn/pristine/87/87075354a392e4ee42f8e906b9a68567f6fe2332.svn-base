...
- (void)loadDataFormNetwork
{
    ...
    [YDDataService startRequest:dic url:getMessageUrl block:^(id result) {
        ...
        if ([result[@"code"] isEqualToString:@"S_OK"]) {
            ...
            NSString *modelId = [resultDic valueForKey:@"corpId"];
            
            if (![modelId isEqualToString:@"INIT"])
            {
            ...
            setNSUser(_custLevelDic, kCustomerLevelKey);
        }
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    [YDDataService startRequest:dic url:getMessageUrl block:^(id result) {
        ...
        if ([result[@"code"] isEqualToString:@"S_OK"]) {
            ...
            [MBProgressHUD showTips:[NSString stringWithFormat:@"修改失败:%@",result[@"code"]] toView:self.view];
        }
