...
@property (nonatomic, strong) YDCustListModel *orderCustomerModel; // 客户查询查到订单客户
...
- (void)goNewFollowup_phoneNO:(NSString *)phoneNO{
    ...
    if (_selectPModel.hasQuote) {
        ...
        //如果有订单客户，带入订单客户信息
        if ([_orderCustomerModel.customerType integerValue] == 2) {
            vc.orderCustomerModel = _orderCustomerModel;
        }
...
- (void)loadPassengersFromNetwork{
    ...
    [YDDataService startSyncRequest:@{} url:url block:^(id result) {
        ...
        NSString *statusCode = [result valueForKey:@"code"];
        
        if ([statusCode isEqualToString:@"S_OK"]) {
            ...
            [self isNeedAgainLoginWithErrorCode:statusCode];
        }
    } failBlock:^(id error) {
...
#warning 检查是否是订单客户（后来加的需求）
            NSDictionary *orderDic = [[result objectForKey:@"var"] objectForKey:@"customerInfo"];
            _orderCustomerModel = [YDCustListModel instanceWithDict:orderDic];
            if ([_orderCustomerModel.customerType integerValue] == 2) {
                //如果是订单客户直接出线下一步
                _enterPhoneNOView.nextButton.hidden = NO;
                return ;
