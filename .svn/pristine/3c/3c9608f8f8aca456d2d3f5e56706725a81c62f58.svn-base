...
- (void)setupViewData
{
    BOOL isQuote = YES; //是否报价
    if (_followInfoModel.eventType.length > 0) {
        NSString *b = [_followInfoModel.eventType substringToIndex:1];
    ...
    for (NSArray *sectionArray in _cellArray) {
        for (YDCustCellModel *cellModel in sectionArray) {
            if ([cellModel.title isEqualToString:@"跟进时间"]) {
                cellModel.subTitle = _followInfoModel.currFollowTime;
                ...
                cellModel.subTitle = _followInfoModel.nextFollowTime;
                          ...
                          block:^(id result) {
                              ...
                              if ([result[@"code"] isEqualToString:@"S_OK"]) {
                                  ...
                                  for (NSDictionary *dic in resultArray) {
                                      ...
                                      cellModel.subTitle = [NSString stringWithFormat:@"¥%.2f", [model.actualBidAmount floatValue]];
