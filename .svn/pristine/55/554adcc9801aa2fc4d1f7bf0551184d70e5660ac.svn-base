...
- (void)createView
{
    ...
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *currentFollowTime = [dateFormatter stringFromDate:[NSDate new]];
    [_followInfoDic setValue:currentFollowTime forKey:@"currFollowTime"];
    
    //如果已存在订单客户，带入基本信息
    if (self.orderCustomerModel) {
...
{
    //第一组信息
    for (YDCustCellModel *firstCellModel in self.cellArray[0]) {
        if ([firstCellModel.title isEqualToString:@"客户名称"]) {
            firstCellModel.subTitle = self.orderCustomerModel.customerName;
            [_customerInfoDic setValue:firstCellModel.subTitle forKey:@"customerName"];
        } else if ([firstCellModel.title isEqualToString:@"联系电话"]) {
            firstCellModel.subTitle = self.orderCustomerModel.customerPhone;
            [_customerInfoDic setValue:firstCellModel.subTitle forKey:@"customerPhone"];
        }
    }
    ...
    if (self.orderCustomerModel.carTypeName.length > 0) {
        NSMutableArray *groupArray = self.cellArray[0];
        YDCustCellModel *isDrive = groupArray[4];
        isDrive.onORoff = YES;
        
        _isDriveModel.subTitle = self.orderCustomerModel.carTypeName;
        [groupArray addObject:_isDriveModel];
    ...
    //第二组信息（基本信息）
    for (YDCustCellModel *secondCellModel in self.cellArray[1]) {
        if ([secondCellModel.title isEqualToString:@"出生日期"]) {
            secondCellModel.subTitle = self.orderCustomerModel.birthdayString;
            [_customerInfoDic setValue:secondCellModel.subTitle forKey:@"birthday"];
        }
        else if ([secondCellModel.title isEqualToString:@"身份证号"]) {
            secondCellModel.subTitle = self.orderCustomerModel.idNumber;
            [_customerInfoDic setValue:secondCellModel.subTitle forKey:@"idNumber"];
        }
        else if ([secondCellModel.title isEqualToString:@"微信／QQ"]) {
            secondCellModel.subTitle = self.orderCustomerModel.qq;
            [_customerInfoDic setValue:secondCellModel.subTitle forKey:@"weixin"];
            [_customerInfoDic setValue:secondCellModel.subTitle forKey:@"qq"];
        }
        else if ([secondCellModel.title isEqualToString:@"家庭情况"]) {
            NSInteger familyStatus = [self.orderCustomerModel.familyStatus integerValue];
            if (familyStatus == 1) {
                [_customerInfoDic setValue:@(1) forKey:@"familyStatus"];
                secondCellModel.subTitle = @"100";
            } else if (familyStatus == 2) {
                [_customerInfoDic setValue:@(2) forKey:@"familyStatus"];
                secondCellModel.subTitle = @"010";
            } else if (familyStatus == 3) {
                [_customerInfoDic setValue:@(3) forKey:@"familyStatus"];
                secondCellModel.subTitle = @"001";
            }
        }
        else if ([secondCellModel.title isEqualToString:@"居住地址"]) {
            secondCellModel.subTitle = self.orderCustomerModel.address;
            [_customerInfoDic setValue:secondCellModel.subTitle forKey:@"address"];
        }
    }
