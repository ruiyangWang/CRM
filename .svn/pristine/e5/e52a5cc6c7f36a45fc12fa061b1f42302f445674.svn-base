...
-(void)myTouch:(NSSet *)touches
{
    ...
    //显示左边的
    if (self.oneTableView.x == 0)
    {
        [self.oneTableView  scrollToRowAtIndexPath:indpath atScrollPosition:UITableViewScrollPositionTop animated:NO];
    }
    else if (self.twoTableView.x == 0)
    {
        [self.twoTableView  scrollToRowAtIndexPath:indpath atScrollPosition:UITableViewScrollPositionTop animated:NO];
    }
