...
- (IBAction)nextBtnClick:(UIButton *)sender {
    if ([_customerModel.customerType integerValue] == 2) {
        if ([self.delegate respondsToSelector:@selector(addCustomerWithOrderCustomer:)]) {
            [self.delegate addCustomerWithOrderCustomer:_customerModel];
        }
    } else {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField.text.length == 10 || string.length == 11) {
...
- (void)setCustomerModel:(YDCustListModel *)customerModel
{
    ...
    if (customerModel) {
        ...
        //订单客户
        if ([customerModel.customerType integerValue] == 2) {
